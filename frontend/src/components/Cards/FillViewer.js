  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <p className="text-sm text-gray-500 dark:text-gray-400">
        ...
        {weakMode && <span className="ml-2 text-xs font-medium bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded px-2 py-0.5">{t('viewer.weak')}</span>}
      </p>
      ...
      <div className="w-full max-w-lg bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 space-y-4">
        ...
        {!submitted ? (
          ...
          <div className={`rounded-xl p-4 ${isCorrect ? 'bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800'}`} aria-live="polite">
            <p className={`text-sm font-semibold ${isCorrect ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('viewer.yourAnswer')} {inputValue}</p>
            {!isCorrect && card.context && (
              <p className="text-sm text-blue-600 dark:text-blue-400 mt-2 italic">💡 {card.context}</p>
            )}
          </div>
        )}
      </div>
